...
class Article extends BaseModel
{
    ...
    {
        return ArticleSubcate::where('id', $data['article_subcate_id'])->value('name');
    }
    
    /**
     * @notes  获取一级code（只返回显示状态的）
     */
    public function getContinentAttr($value, $data)
    {
        // 查询分类信息并检查显示状态
        $cate = ArticleCate::where('id', $data['cid'])
            ->field('code, is_show')
            ->find();
        
        // 只有显示状态为1时才返回code，否则返回空或null
        if ($cate && $cate['is_show'] == 1) {
            return $cate['code'];
        }
        
        return ''; // 或者返回 null，根据你的需求
    }
    
    /**
     * @notes  获取二级code（只返回显示状态的）
     */
    public function getCountryAttr($value, $data)
    {
        // 查询二级分类信息并检查显示状态
        $subcate = ArticleSubcate::where('id', $data['article_subcate_id'])
            ->field('code, is_show')
            ->find();
        
        // 只有显示状态为1时才返回code
        if ($subcate && $subcate['is_show'] == 1) {
            return $subcate['code'];
        }
