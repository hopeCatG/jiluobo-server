  ...
  public function lists(): array
{
    ...
    $field = 'id,cid,title,image,click_virtual,click_actual,article_subcate_id,city,latLng,author,post_time';
    ...
    $result = Article::field($field)
        ...
        ->whereIn('cid', function($query) {
            $query->name('article_cate')->where('is_show', 1)->field('id');
        })
        ->whereIn('article_subcate_id', function($query) {
            $query->name('article_subcate')->where('is_show', 1)->field('id');
        })
        ->orderRaw($orderRaw)
        ->append(['country', 'continent'])
        ->hidden(['click_virtual', 'click_actual', 'article_subcate_id', 'cid'])
